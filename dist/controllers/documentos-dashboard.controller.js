...
exports.documentosDashboardController = void 0;
...
exports.documentosDashboardController = {
    // KPIs Ejecutivos
    async getKPIs(req, res) {
        try {
            const placa = req.query.placa;
            const area_operacion = req.query.area_operacion;
            let query = supabase_1.supabase.from('documentos_vehiculos_relaciones').select('*');
            if (placa)
    ...
    async getCalendarEvents(req, res) {
        try {
            ...
            let query = supabase_1.supabase.from('documentos_vehiculos_relaciones')
            ...
            data?.forEach((row) => {
                documentTypes.forEach(docType => {
                    ...
                    if (tipo_documento) {
                        ...
                        if (types.length > 0 && !types.includes(docType.label))
                            return;
                    }
                    ...
                    if (fechaDate < startOfMonth || fechaDate > endOfMonth)
                        return;
    ...
    async getExpiringList(req, res) {
        try {
            const placa = req.query.placa;
            const area_operacion = req.query.area_operacion;
            const tipo_documento = req.query.tipo_documento;
            const estado_filter = req.query.estado;
            ...
            data?.forEach((row) => {
                documentTypes.forEach(docType => {
                    ...
                    if (estado_filter) {
                        ...
                        if (states.length > 0 && !states.includes(estado))
                            return;
                    }
                    items.push({
                        id: `${row.id}-${docType.key}`,
        ...
        try {
            ...
            data?.forEach((row) => {
                ...
                documentTypes.forEach(docType => {
                    ...
                    switch (estado) {
                        case 'VENCIDO':
                            byArea[area].vencidos++;
                            break;
                        case 'POR_VENCER':
                            byArea[area].porVencer++;
                            break;
                        case 'PROXIMO':
                            byArea[area].proximos++;
                            break;
                        case 'VIGENTE':
                            byArea[area].vigentes++;
                            break;
                    }
                });
            });
            const result = Object.values(byArea).sort((a, b) => (b.vencidos + b.porVencer) - (a.vencidos + a.porVencer));
            res.json(result);
        }
        catch (error) {
