...
export const tanqueosController = {
    ...
    async getAll(req: AuthRequest, res: Response): Promise<void> {
        try {
            ...
            if (bomba) summaryQuery = applyFilter(summaryQuery, 'bomba', bomba);
            if (area_operacion) summaryQuery = applyFilter(summaryQuery, 'area_operacion', area_operacion);
            if (tipo_combustible) summaryQuery = applyFilter(summaryQuery, 'tipo_combustible', tipo_combustible);
            if (concepto) summaryQuery = applyFilter(summaryQuery, 'concepto', concepto);
            if (tipo_operacion) summaryQuery = applyFilter(summaryQuery, 'tipo_operacion', tipo_operacion);
    ...
    async getFilterOptions(req: AuthRequest, res: Response): Promise<void> {
        try {
            ...
            if (hasFilters) {
                ...
                let query: any = dbClient.from('tanqueo_relaciones').select('conductor, placa, bomba, area_operacion, tipo_combustible, concepto, tipo_operacion');
                query = await applyActiveAreasFilter(query, dbClient);
    ...
    async create(req: AuthRequest, res: Response): Promise<void> {
        try {
            ...
            if (!req.user?.isCierreAdmin && req.body.fecha) {
                ...
                const monthClosed = await isMonthClosed(checkClient, 'tanqueo', req.body.fecha);
                if (monthClosed) {
    ...
    async getFinancialReport(req: AuthRequest, res: Response): Promise<void> {
        try {
            ...
            if (placa) query = applyFilter(query, 'Placa', placa);
            if (bomba) query = applyFilter(query, 'Tercero', bomba);
            if (area_operacion) query = applyFilter(query, 'Área de Operacion', area_operacion);
            if (concepto) query = applyFilter(query, 'Concepto', concepto);
            if (tipo_combustible) query = applyFilter(query, 'Sub-Rubro', tipo_combustible);

            const { names: deactivatedAreaNames } = await getDeactivatedAreas(req.supabase);
            if (deactivatedAreaNames.length > 0) {
                query = query.not('Área de Operacion', 'in', `(${deactivatedAreaNames.map((n: string) => `"${n}"`).join(',')})`);
            }
    ...
    async getExportData(req: AuthRequest, res: Response): Promise<void> {
        try {
            ...
            let query: any = (req.supabase || supabase).from('tanqueo_relaciones').select('*');
