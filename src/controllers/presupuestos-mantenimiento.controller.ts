...
export const presupuestosMantenimientoController = {
  async getFilterOptions(req: AuthRequest, res: Response) {
    try {
      ...
      const { data: vehiculosRaw } = await dbClient.from("vehiculo").select(`
                    id,
                    operacion_id,
      ...
      const vehiculos =
        vehiculosRaw
          ...
          .map((v: any) => ({
            ...
            clase_vehiculo:
              v.vehiculo_caracteristicas?.cat_clase_vehiculo?.nombre || null,
  ...
  async getAll(req: AuthRequest, res: Response): Promise<void> {
    try {
      ...
      let query = dbClient.from("presupuesto_unificado").select(
        `
                    *,
      ...
      if (
        ...
        if (isNum(filterPlaca)) {
          ...
          const { data: placasData } = await dbClient
            .from("areas_placas")
            .select("id")
            .ilike("placa", `%${String(filterPlaca).trim()}%`);
          const pIds = placasData?.map((p) => p.id) || [];
          if (pIds.length > 0) {
            const { data: vehData } = await dbClient
              .from("vehiculo")
              .select("id")
              .in("placa_id", pIds);
            filterVehiculosIds =
              vehData && vehData.length > 0 ? vehData.map((v) => v.id) : [-1];
          } else {
      ...
      const filterEmpresasIds = await getFilterIds(
        "empresas",
        "empresa",
        empresa,
      );
      const filterAreasIds = await getFilterIds(
        "areas_operacion",
        "nombre",
        area_operacion,
      );
      const filterGruposIds = await getFilterIds(
        "maestro_rubros",
        "nombre",
        grupo_rubro,
        { col: "nivel", val: 2 },
      );
      const filterRubrosIds = await getFilterIds(
        "maestro_rubros",
        "nombre",
        rubro,
        { col: "nivel", val: 3 },
      );
      const filterSubRubrosIds = await getFilterIds(
        "tipos_presupuesto",
        "nombre",
        sub_rubro,
      );
      const filterConceptosIds = await getFilterIds(
        "conceptos_presupuesto",
        "nombre",
        concepto,
      );

      if (filterVehiculosIds)
        query = query.in("vehiculo_id", filterVehiculosIds);
      ...
      if (filterSubRubrosIds)
        query = query.in("tipo_presupuesto_id", filterSubRubrosIds);
      if (filterConceptosIds)
        query = query.in("concepto_presupuesto_id", filterConceptosIds);
      ...
      if (q && q !== "undefined" && q !== "" && q !== "null") {
        ...
        const { data: qConceptosData } = await dbClient
          .from("conceptos_presupuesto")
          .select("id")
          .ilike("nombre", `%${searchTerm}%`);
        const qConceptosIds = qConceptosData?.map((c) => c.id) || [];

        const { data: qTiposData } = await dbClient
          .from("tipos_presupuesto")
          .select("id")
          .ilike("nombre", `%${searchTerm}%`);
        const qTiposIds = qTiposData?.map((c) => c.id) || [];

        const { data: qRubrosData } = await dbClient
          .from("maestro_rubros")
          .select("id")
          .ilike("nombre", `%${searchTerm}%`);
        ...
        if (qVehiculosIds.length > 0)
          orConditions.push(`vehiculo_id.in.(${qVehiculosIds.join(",")})`);
        if (qConceptosIds.length > 0)
          orConditions.push(
            `concepto_presupuesto_id.in.(${qConceptosIds.join(",")})`,
          );
        if (qTiposIds.length > 0)
          orConditions.push(`tipo_presupuesto_id.in.(${qTiposIds.join(",")})`);
      ...
      if (error) {
        console.error(
          "❌ Error de Supabase en getAll presupuesto_unificado:",
          error,
        );
      ...
      let summaryQuery = dbClient
        ...
        .select(
          "valor_total, estado, ejecutado, meses_aplicables, valor_unitario, frecuencia_mes, rubro_id",
        );

      if (filterVehiculosIds)
        summaryQuery = summaryQuery.in("vehiculo_id", filterVehiculosIds);
      if (filterEmpresasIds)
        summaryQuery = summaryQuery.in("empresa_id", filterEmpresasIds);
      if (filterAreasIds)
        summaryQuery = summaryQuery.in("area_operacion_id", filterAreasIds);
      if (isNum(anio)) summaryQuery = summaryQuery.eq("anio", Number(anio));
      if (filterGruposIds)
        summaryQuery = summaryQuery.in("grupo_rubro_id", filterGruposIds);
      if (filterRubrosIds)
        summaryQuery = summaryQuery.in("rubro_id", filterRubrosIds);
      if (filterSubRubrosIds)
        summaryQuery = summaryQuery.in(
          "tipo_presupuesto_id",
          filterSubRubrosIds,
        );
      if (filterConceptosIds)
      ...
      try {
        ...
        const { data: presupuestosForValidation } = await dbClient
          .from("presupuesto_unificado")
          .select(
            "grupo_rubro_id, grupo:maestro_rubros!grupo_rubro_id(nombre, codigo_concatenado)",
          )
          .eq("anio", yearNum);
        ...
        if (presupuestosForValidation && presupuestosForValidation.length > 0) {
          ...
          const inRegExp = {
            $in: Array.from(groups).map((g) => new RegExp(g, "i")),
          };
          const orFilters: any[] = [
            { grupoRubro: inRegExp },
        ...
        if (grupo_rubro && grupo_rubro !== "undefined") {
          ...
          if (arr.length > 0) {
            ...
            if (filterGruposIds && filterGruposIds.length > 0) {
              const { data: grData } = await dbClient
                .from("maestro_rubros")
                .select("codigo_concatenado")
                .in("id", filterGruposIds);
              codigos =
                grData?.map((d) => d.codigo_concatenado).filter(Boolean) || [];
            }
        ...
        console.error(
          "❌ Error al calcular totalEjecutadoReal desde MongoDB:",
          mongoError,
        );
      }
  ...
  async getDashboardKPIs(req: AuthRequest, res: Response): Promise<void> {
    const originalJson = res.json.bind(res);
    res.json = function (data: any) {
      return originalJson(data.summary || data);
    } as any;
    return presupuestosMantenimientoController.getAll(req, res);
  },

  async getDashboardMatrix(req: AuthRequest, res: Response): Promise<void> {
    try {
      const dbClient = req.supabase || supabase;
      const { anio } = req.query;
      let query = dbClient
        .from("presupuesto_unificado")
        .select("*, vehiculo(areas_placas(placa)), tipo:tipos_presupuesto(*)");
      if (anio && anio !== "undefined") query = query.eq("anio", Number(anio));

      const { data } = await query;
      const matrixMap = new Map<string, any>();

      (data || []).forEach((row: any) => {
        const placa = row.vehiculo?.areas_placas?.placa || "Sin Placa";
        const tipo = row.tipo?.nombre || "General";
        ...
        record.total_presupuesto += row.valor_total || 0;

        const ejecutado = row.ejecutado === "SI";
        const meses = row.meses_aplicables || [];
        const valPorMes =
          meses.length > 0
            ? row.valor_total / meses.length
            : row.valor_total || 0;
        ...
        meses.forEach((mes: number) => {
          ...
          if (ejecutado) {
            record.meses[mes].ejecutado += valPorMes;
          } else {
            record.meses[mes].presupuestado += valPorMes;
          }
          record.meses[mes].detalles.push({ ...row, valor_mes: valPorMes });
        });
      });

      res.json(Array.from(matrixMap.values()));
    } catch (error) {
      res.status(500).json({ error: "Error" });
    }
  },

  async getDashboardByPlaca(req: AuthRequest, res: Response): Promise<void> {
    try {
      const dbClient = req.supabase || supabase;
      const { anio } = req.query;
      let query = dbClient
        .from("presupuesto_unificado")
        .select("*, vehiculo(areas_placas(placa))");
      if (anio && anio !== "undefined") query = query.eq("anio", Number(anio));
      ...
      (data || []).forEach((row: any) => {
        const placa = row.vehiculo?.areas_placas?.placa || "Sin Placa";
        if (!map.has(placa)) {
          map.set(placa, { placa, presupuestado: 0, ejecutado: 0 });
        }
        const record = map.get(placa);
        const isEjecutado = row.ejecutado === "SI";
        if (isEjecutado) record.ejecutado += row.valor_total || 0;
        else record.presupuestado += row.valor_total || 0;
      });

      const result = Array.from(map.values()).map((r) => ({
        ...r,
        total_presupuesto: r.presupuestado + r.ejecutado,
        diferencia: r.presupuestado - r.ejecutado,
        porcentaje_ejecucion:
          r.presupuestado > 0
            ? (r.ejecutado / r.presupuestado) * 100
            : r.ejecutado > 0
              ? 100
              : 0,
      }));
  ...
  async getDashboardByEmpresa(req: AuthRequest, res: Response): Promise<void> {
    try {
      const dbClient = req.supabase || supabase;
      const { anio } = req.query;
      let query = dbClient
      ...
      (data || []).forEach((row: any) => {
        const empresa = row.empresas?.empresa || "Sin Empresa";
        if (!map.has(empresa)) {
          map.set(empresa, { empresa, presupuestado: 0, ejecutado: 0 });
        }
        const record = map.get(empresa);
        const isEjecutado = row.ejecutado === "SI";
        if (isEjecutado) record.ejecutado += row.valor_total || 0;
        else record.presupuestado += row.valor_total || 0;
      });
      ...
      if (items && Array.isArray(items) && items.length > 0) {
        const rowsToInsert = items.map((item: any) => ({
          ...
          valor_total:
            item.valor_unitario *
            item.frecuencia_mes *
            (item.meses_aplicables?.length || 0),
          nota: item.nota,
        ...
        if (
          ...
          payload.frecuencia_mes
        ) {
          const numMeses = (payload.meses_aplicables || []).length;
          payload.valor_total =
            payload.valor_unitario * payload.frecuencia_mes * numMeses;
        }
        const { data, error } = await dbClient
          .from("presupuesto_unificado")
          .insert(payload)
  ...
  async update(req: AuthRequest, res: Response): Promise<void> {
    try {
      ...
      if (items && Array.isArray(items) && items.length > 0) {
        ...
        const frecuencia_mes =
          payload.frecuencia_mes !== undefined
            ? payload.frecuencia_mes
            : current.frecuencia_mes;
        const meses_aplicables =
          payload.meses_aplicables !== undefined
            ? payload.meses_aplicables
            : current.meses_aplicables;

        payload.valor_total =
          valor_unitario * frecuencia_mes * (meses_aplicables?.length || 0);
